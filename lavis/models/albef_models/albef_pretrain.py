...
class AlbefPretrain(AlbefBase, MomentumDistilationMixin, SharedQueueMixin):
    ...
    def forward(self, samples):
        ...
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to( #(bs,1+um_patch)
            self.device
        )
        ...
        with torch.no_grad():
            ...
            image_feat_all = torch.cat(
                [image_feat_m.t(), self.image_queue.clone().detach()], dim=1#image_feat_m.t：[dim,bs] s
            )
            ...
            text_feat_m = F.normalize(self.text_proj_m(text_embeds_m[:, 0, :]), dim=-1) #(bs,256) 256 is feature dimention
            ...
            sim_targets.fill_diagonal_(1) #自己和自己的相似度是1，这就是label
        ...
        '''此处的loss是和queeze里面所有的'''
        loss_i2t = -torch.sum(
            F.log_softmax(sim_i2t, dim=1) * sim_i2t_targets, dim=1#todo 为什么只有交叉熵的半项？
        ).mean()
        loss_t2i = -torch.sum(
        ...
        with torch.no_grad():
            ...
            weights_t2i = sim_t2i[:, :bs].clone()#t[bs,bs],sim_t2i最前bs个是当前bs新加入到queue中的

            weights_i2t.fill_diagonal_(-np.Inf)
            weights_t2i.fill_diagonal_(-np.Inf)

            weights_i2t = F.softmax(weights_i2t, dim=1)
            weights_t2i = F.softmax(weights_t2i, dim=1)#-inf 经过softmax后变成0

        # select a negative image for each text
        image_embeds_neg = []
        for b in range(bs):
            neg_idx = torch.multinomial(weights_t2i[b], 1).item() #todo 为什么是multinomial采样，而不是直接选取weight最大的？
            image_embeds_neg.append(image_embeds[neg_idx])
        image_embeds_neg = torch.stack(image_embeds_neg, dim=0)#(bs,257,hiddensize)
        ...
        text_atts_neg = torch.stack(text_atts_neg, dim=0) #text_atts_neg:record selected neg text's attention mask(0 rep padding tocken)
        '''
        注意！text_embeds_all and image_atts_all's components are corresponded.text_embeds和image_embeds_neg对应，text_embeds_neg和image_embeds对应
        '''
        text_embeds_all = torch.cat([text_embeds, text_embeds_neg], dim=0)
        text_atts_all = torch.cat([text.attention_mask, text_atts_neg], dim=0)

        image_embeds_all = torch.cat([image_embeds_neg, image_embeds], dim=0)
        image_atts_all = torch.cat([image_atts, image_atts], dim=0)# image_atts: all one

        encoder_output_neg = self.text_encoder.bert(
            encoder_embeds=text_embeds_all,
            attention_mask=text_atts_all,
            encoder_hidden_states=image_embeds_all,# it means the feature is from encoder, or cross-attention's kv
        ...
        itm_labels = torch.cat(
            [torch.ones(bs, dtype=torch.long), torch.zeros(2 * bs, dtype=torch.long)],#todo 为什么是2*
            dim=0,
        ).to(self.device)
        ...
        mlm_output = self.text_encoder(#text_encoder:bert+classifier
            input_ids,
            attention_mask=text.attention_mask,
            encoder_hidden_states=image_embeds,
            encoder_attention_mask=image_atts,
    ...
    def from_config(cls, cfg=None):
        ...
        text_encoder = BertForMaskedLM.from_pretrained(#load checkpoint for bert pretraining
