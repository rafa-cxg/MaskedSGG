...
class SggpFinetune(SggpBase):
    ...
    def __init__(
        ...
        if mode == "contrastive":
            ...
            self.vision_proj = nn.Linear(9*768, 768)
        ...
        if num_classes > 0:
            ...
            self.cls_head1 = nn.Linear(768, 768)
            self.cls_head =  nn.Linear(768, 51)
    ...
    def forward(self, samples, is_train=True):
        ...
        for i,(image,sentance_tocken,batch_relation_targets,instance_gt) in enumerate(zip(samples['image'], samples["sentance_tockens"],relation_targets,instance_gts)):
            ...
            if self.predict_mode =="contrastive":
                ...
                temp = []
                # torch.select(encoder_output.hidden_states[-1],1,torch.add(relation_index,197))
                for B, index in enumerate(torch.add(relation_index,197)):
                    text_relation_feature = encoder_output.hidden_states[B,  index,
                                            :].unsqueeze(0)  # (num_rel,1,hidden_size)
                    temp.append(text_relation_feature)
                text_relation_feature = torch.cat(temp,0)
                text_relation_feature =  F.gelu(text_relation_feature)
                ...
                # text_relation_feature = (
                #             text_relation_feature / text_relation_feature.norm(dim=1, keepdim=True))
                ...
                if is_train:
                    ...
                    logits = self.cls_head(F.gelu(self.cls_head1(visual_relation_feature)))
                    contrastive_loss = F.cross_entropy(logits, batch_relation_targets.view(-1))
