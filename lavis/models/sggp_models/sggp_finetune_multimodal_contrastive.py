...
class SggpFinetune(SggpBase):
    ...
    def __init__(
        ...
        if mode == "contrastive":
            ...
            self.vision_proj = nn.Linear(9, 1)
    ...
    def forward(self, samples, is_train=True):
        ...
        for instance_gt in instance_gts:
            ...
            for triplet in instance_gt.get("relation_tuple"):
                ...
                if self.predict_mode == "contrastive":
                    # 准备text prompt
                    if is_train:
                        # text_inputs = "the relation between {} and {} are [MASK] ".format(
                        ...
                        # negative_text_set = [f"the relation between two objects are {c} " for c in self.predicate_to_idx.keys() if c != self.idx_to_predicate[str(int(triplet[2]))] ]

                        sentance_tockens.append(text_inputs)
                        # negative_text_sets.append(negative_text_set)
                    else:
                        # text_inputs = [f"the relation between {self.idx_to_label[str(int(label[int(triplet[0])]))]} and " \
                ...
                if self.predict_mode == "contrastive":
                    ...
                    xmin = floor((max(box1[0], box2[0])) / 16*ratio_scale[0])
                    xmax = ceil((min(box1[2], box2[2])) /  16*ratio_scale[0])
                    ymin = floor((max(box1[1], box2[1])) / 16*ratio_scale[1])
                    ymax = ceil((min(box1[3], box2[3])) /  16*ratio_scale[1])
                    if (xmax - xmin) == 0:
                        xmax = xmin + 1
                    if (ymax - ymin) == 0:
                        ymax = ymin + 1

                    union_box = torch.tensor((xmin, ymin, xmax, ymax),dtype=torch.long)
        ...
        predictions = []
        prediction = []
        visual_relation_features = []
        text_relation_features = []
        negative_relation_features = []
        for i,(image,sentance_tocken,batch_relation_targets,instance_gt) in enumerate(zip(samples['image'], samples["sentance_tockens"],relation_targets,instance_gts)):
            ...
            try:
                relation_index = torch.where(tokenized_texts["input_ids"][0] == 103)[0]  # relation_index是relation词出现的位置 #todo 考虑batch中可能relation出现位置不一致,这先按照一致计算
            ...
            if self.predict_mode =="contrastive":
                interactive_box = interactive_boxes[i]
                # 1为匹配，0为负样本
                ...
                if is_train:
                    ...
                    interactive_box = torch.cat((torch.arange(0, interactive_box.shape[0]).unsqueeze(1).expand(
                        interactive_box.shape[0], -1), interactive_box), 1)

                    visual_relation_feature = ROIAlign((3, 3), 1, 0, aligned=True).forward(
                        encoder_output.hidden_states[-1][:, 1:197, :].reshape(-1, 14, 14, 768).permute(0, 3, 1, 2),
                        interactive_box.cuda()).squeeze(1)
                    text_relation_feature = encoder_output.hidden_states[-1][:, 197 + relation_index,:]  # (num_rel,1,hidden_size)

                    # normalized features
                    visual_relation_feature = self.vision_proj(
                        visual_relation_feature.reshape(*visual_relation_feature.shape[:-2], -1))
                    visual_relation_feature = (
                                visual_relation_feature / visual_relation_feature.norm(dim=1, keepdim=True)).squeeze(2)
                    text_relation_feature = (text_relation_feature / text_relation_feature.norm(dim=1, keepdim=True)).squeeze(1)


                    visual_relation_features.append(visual_relation_feature)
                    text_relation_features.append(text_relation_feature)



                    #calculate loss
                    # labels = torch.arange(visual_relation_feature.shape[0], device='cuda').long()
                    # contrastive_loss = F.cross_entropy(logits_per_image, labels)
                    # contrastive_losses.append(contrastive_loss)

                if is_train ==False:
                    num_rel = encoder_output.logits.shape[0]
                    # patch pooling
                    prediction = []
                    ...
                    predictions.append(torch.stack(prediction,0))
        ...
        if self.predict_mode == "contrastive" and is_train:
            ...
            for i,relation_target in enumerate(relation_targets):
                ...
                logits_per_image = torch.cat((logits_per_rel[i,i].unsqueeze(0),logits_per_rel[i, padding_mask == 0]),0).unsqueeze(0)
        ...
        if is_train:
            ...
            if self.predict_mode == "contrastive":
                contrastive_losses = contrastive_loss / len(samples["image"])
