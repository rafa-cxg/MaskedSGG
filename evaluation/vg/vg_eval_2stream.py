...
from evaluation.coco.coco_eval import COCOResults

import logging
# from evaluation.coco.coco_eval import COCOResults
from evaluation.vg.sgg_eval_for_2stream import SGRecall, SGNoGraphConstraintRecall, \
    SGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall
...
eval_times = 0
MAX_PROCESS = 2
map_predicate2bert_idx = torch.load(os.path.abspath('map_predicate2bert_idx.pth'))
LONGTAIL_PART_DICT = [None, 'b', 't', 't', 't', 't', 't', 't', 'b', 't', 't', 't', 't', 't',
...
def do_vg_evaluation(
    ...
    num_rel_category = 51
    try:#sgdet
        if predictions[0][0].get('rel_scores').shape[-1]>51:#说明是zero-shot
            num_rel_category = 30522
    except: #predcls
        if predictions[0][0].shape[-1]>51:
            num_rel_category = 30522
    if num_rel_category>51:
        from transformers import BertTokenizer
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        all_tokens = tokenizer.get_vocab().keys()
        # 转换为列表并打印
        all_tokens_list = list(all_tokens)
        with open("ind_to_predicates_bert", 'w') as f:
            json.dump(all_tokens_list, f)
    ...
    if num_rel_category==51:
        with open("ind_to_predicates","r") as f:
            ind_to_predicates=json.load(f)
    else: # bert-vocabulary for mrecall statistics
        with open("ind_to_predicates_bert","r") as f:
            ind_to_predicates=json.load(f)
    # ind_to_predicates[0]='background'
    with open("ind_to_classes","r") as f:
        ind_to_classes=json.load(f)
    ...
    if "bbox" in iou_types and mode!="predcls": #对于predcls，不需要评估bbox
        ...
        for image_id, prediction in enumerate(predictions):
            ...
            box = BoxMode.convert(prediction.pred_boxes.tensor, BoxMode.XYXY_ABS,BoxMode.XYWH_ABS).detach().cpu().numpy()
            # box = prediction.convert('xywh').bbox.detach().cpu().numpy()  # xywh
            score = prediction.scores.detach().cpu().numpy()  # (#objs,)
            label = prediction.pred_classes.detach().cpu().numpy()  # (#objs,)
        ...
        result_str += 'Detection evaluation mAp=%.8f\n' % mAp
        result_str += "recall@%d IOU:0.5 %.4f\n" % get_coco_eval(coco_eval, 0.5, 'recall')
        result_str += '=' * 100 + '\n'
        # avg_metrics = mAp
        logging.info(result_str)
        result_str = '\n'
        logging.info("box evaluation done!")
    ...
    if "relations" in iou_types:  # rel_eval_result_dict存metric结果，evaluater存各种recall的函数
        ...
        eval_mean_recall = SGMeanRecall(rel_eval_result_dict, num_rel_category, ind_to_predicates,
                                        print_detail=True)
        eval_mean_recall.register_container(mode)
        evaluator['eval_mean_recall'] = eval_mean_recall  # 倒来倒去...即evaluater里面的函数对应着eval_mean_recall

        # used for NG-meanRecall@K
        eval_ng_mean_recall = SGNGMeanRecall(result_dict, num_rel_category, ind_to_predicates,
                                             print_detail=True)
        ...
        logging.info("evaluating relationship predictions..")
        ...
        for groundtruth, prediction in tqdm(zip(groundtruths, predictions),
                                            total=len(predictions)):  # 'relation_tuple':[sub_id\obj_id\rel]
            evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator,
                                           )  # local_container再此创建。该函数中，evaluater存储的各类metric函数库，实际计算结果都在每个函数的self.result_dict里面，已append list方式
        ...
        def longtail_part_eval(evaluator, mode):
            longtail_part_dict =LONGTAIL_PART_DICT
        ...
        # 目前我只想保留mean recall
        avg_metrics =0# np.mean(rel_eval_result_dict[mode + '_mean_recall'][100]+rel_eval_result_dict[mode + '_recall'][100])#recall+mrecall100
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    因为pysgg只支持单一图片为evaluate单位，而groundth是batchsize大小的list，这里要求eval_batch应为1'''


    assert len(groundtruth)==1
    groundtruth=groundtruth[0]
    prediction = prediction[0]
    try:
        if(prediction.get('rel_scores').shape[-1])>51:
            update_predicate = torch.tensor(
                [map_predicate2bert_idx[int(item)] for item in groundtruth.get('relation_tuple')[:, 2]],
                dtype=torch.long).unsqueeze(-1)
            temp = torch.cat((groundtruth.get('relation_tuple')[:, :2].cpu(), update_predicate.cpu()), -1)
            groundtruth.set('relation_tuple', temp)
    except:#predcls

        if (prediction.shape[-1]) > 51:
            update_predicate = torch.tensor(
                [map_predicate2bert_idx[int(item)] for item in groundtruth.get('relation_tuple')[:, 2]],
                dtype=torch.long).unsqueeze(-1)
            temp = torch.cat((groundtruth.get('relation_tuple')[:, :2].cpu(), update_predicate.cpu()), -1)
    ...
    local_container['gt_rels'] = groundtruth.get('relation_tuple').long().detach().cpu().numpy()  # [23，3]

    # if there is no gt relations for current image, then skip it
    if len(local_container['gt_rels']) == 0:
        return

    local_container['gt_boxes'] = groundtruth.get('gt_boxes').tensor.detach().cpu().numpy()  # (#gt_objs, 4)
    local_container['gt_classes'] = groundtruth.get('labels').long().detach().cpu().numpy()  # (#gt_objs, )

    # about relations

    if mode == "predcls":
        local_container['pred_rel_inds'] = groundtruth.get(#
            'rel_pair_idxs').long().detach().cpu().numpy()
        # local_container['pred_rel_inds'] = groundtruth.get(
        #     'relation_tuple')[:,:-1].long().detach().cpu().numpy()  # sgdet:(#pred_rels, 2) eg(4096,2) .predcls:[num_box!,2]
        local_container['rel_scores'] = prediction.detach().cpu().numpy()  # (#pred_rels, num_pred_class)
    else:  # (#pred_rels, num_pred_class)
        # about relations
        local_container['rel_scores'] = prediction.get('rel_scores').detach().cpu().numpy()  # (#pred_rels, num_pred_class)
        local_container['pred_rel_inds'] = prediction.get(
            'rel_pair_idxs').long().detach().cpu().numpy()  # sgdet:(#pred_rels, 2) eg(4096,2) .predcls:[num_box!,2]
        # about objects
        local_container['pred_boxes'] = prediction.pred_boxes.tensor.detach().cpu().numpy()  # (#pred_objs, 4) ifpredcls:数值和gt_boxes一样
        local_container['pred_classes'] = prediction.pred_classes.long().detach().cpu().numpy()  # (#pred_objs, )
        local_container['obj_scores'] = prediction.scores.detach().cpu().numpy()  # (#pred_objs, )
