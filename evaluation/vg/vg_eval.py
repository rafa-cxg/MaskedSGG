...
from evaluation.vg.sgg_eval import SGRecall, SGNoGraphConstraintRecall, \
    SGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall
from lavis.common.registry import registry
from lavis.datasets.datasets.vg_sgg_datasets import HEAD, TAIL, BODY
# from  lavis.configs.default_config import _C as yacs_config
...
LONGTAIL_PART_DICT = [None, 'b', 't', 't', 't', 't', 't', 't', 'b', 't', 't', 't', 't', 't',
                                                 ...
                                                 't', 'b', 't', 't', 't', 't', 'h', 'b', 'b']
# yacs_config.merge_from_file("lavis/projects/sggp/train/finetune.yaml")
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    temp = torch.cat((groundtruth.get('relation_tuple')[:, :2],update_predicate),-1)
    ...
    if isinstance(prediction,Instances): #数据类型为instance时候说明是sggcls或者sgdet
        # about relations
        local_container['pred_rel_inds'] = prediction.get(
            'relation_tuple')[:,:-1].long().detach().cpu().numpy()  # sgdet:(#pred_rels, 2) eg(4096,2) .predcls:[num_box!,2]
        local_container['rel_scores'] = prediction.detach().cpu().numpy()  # (#pred_rels, num_pred_class)
        # about objects
        local_container['pred_boxes'] = prediction.convert(
            'xyxy').bbox.detach().cpu().numpy()  # (#pred_objs, 4) ifpredcls:数值和gt_boxes一样
